...
abstract class SocketWorker extends AbstractWorker {
    ...
    public function __construct($worker_name = null) {
        ...
        // 接收缓冲区大小限制
        if (($max_recv_buffer_size = Lib\Config::get($this->workerName . '.max_recv_buffer_size'))
        ...
        // 发送缓冲区大小限制
        if (($max_send_buffer_size = Lib\Config::get($this->workerName . '.max_send_buffer_size'))
    ...
    public function setEventLoopName($event_loop_name) {
        ...
        require_once WORKERMAN_ROOT_DIR . 'Core/Events/' . ucfirst(str_replace('WORKERMAN', '', $event_loop_name))
            . '.php';
    }
    ...
    public function dealInputBase($connection, $flag, $fd = null) {
        ...
        elseif ($remain_len > 0) {
            ...
            if (strlen($this->recvBuffers[$fd]['buf']) + $remain_len > $this->maxRecvBufferSize) {
                $this->notice(
                    'client_ip:' . $this->getRemoteIp()
                    . ' send a packet which length greater than conf.max_recv_buffer_size:' . $this->maxRecvBufferSize
                    . ' .May be an attack so close connection'
                );
                $this->closeClient($fd);
            }
            else {
    ...
    public function sendToClient($buffer_to_send) {
        ...
        if ($this->protocol != 'udp') {
            if (!empty($this->sendBuffers[$this->currentDealFd])) {
                ...
                if ($total_send_buffer_len > $this->maxSendBufferSize) {
                    $this->notice(
                        'sendToClient fail. The send buffer\'s length of client_ip:' . $this->getRemoteIp() . ' is '
                        . $total_send_buffer_len . ' greater than conf.d.max_send_buffer_size:'
                        . $this->maxSendBufferSize . ', so discard the packet'
                    );
            ...
            if (!isset($this->connections[$this->currentDealFd])) {
                $debug_str = new \Exception(
                    'sendToClient fail. Connections of ' . $this->currentDealFd . ' does not exist'
                );
            ...
            $this->event->add(
                $this->connections[$this->currentDealFd], Events\BaseEvent::EV_WRITE, array($this, 'tcpWriteToClient')
            );
        ...
        return strlen($buffer_to_send) == stream_socket_sendto(
            $this->mainSocket, $buffer_to_send, 0, $this->currentClientAddress
        );
    }
    ...
    protected function writeStatusToQueue() {
        ...
        $error_code = 0;
        @msg_send(
            Master::getQueueId(), self::MSG_TYPE_STATUS, array_merge(
                $this->statusInfo,
                array('memory' => memory_get_usage(true), 'pid' => posix_getpid(), 'worker_name' => $this->workerName)
    ...
    protected function writeFilesListToQueue() {
        ...
        if ($file_list) {
            foreach (array_chunk($file_list, 10, true) as $list) {
                @msg_send(
                    Master::getQueueId(), self::MSG_TYPE_FILE_MONITOR, array_keys($list), true, false, $error_code
                );
            }
        }
    }

...
class Master {
     ...
     * @var array
     */
    protected static $serviceStatusInfo
        = array(
            'start_time' => 0,
    ...
    public static function savePid() {
        ...
        if (false === @file_put_contents(WORKERMAN_PID_FILE, self::$masterPid)) {
            exit("\033[31;40mCan not save pid to pid-file(" . WORKERMAN_PID_FILE
    ...
    protected static function createListeningSockets() {
        ...
        foreach (Lib\Config::getAllWorkers() as $worker_name => $config) {
            if (isset($config['listen'])) {
                $flags = substr($config['listen'], 0, 3) == 'udp'
                    ? STREAM_SERVER_BIND : STREAM_SERVER_BIND | STREAM_SERVER_LISTEN;
                $error_no = 0;
                $error_msg = '';
                // 创建监听socket
                self::$listenedSocketsArray[$worker_name] = stream_socket_server(
                    $config['listen'], $error_no, $error_msg, $flags
                );
                if (!self::$listenedSocketsArray[$worker_name]) {
                    Lib\Log::add(
                        "can not create socket {$config['listen']} info:{$error_no} {$error_msg}\tServer start fail"
                    );
    ...
    public static function checkWorkerExit() {
        ...
        while (($pid = pcntl_waitpid(-1, $status, WUNTRACED | WNOHANG)) != 0) {
            ...
            // 记录进程退出状态
            self::$serviceStatusInfo['worker_exit_code'][$worker_name][$status]
                = isset(self::$serviceStatusInfo['worker_exit_code'][$worker_name][$status]) ?
                self::$serviceStatusInfo['worker_exit_code'][$worker_name][$status] + 1 : 1;
            // 更新状态到共享内存
            self::updateStatusToShm();
    ...
    public static function restartPids() {
        ...
        foreach (self::$pidsToRestart as $pid => $stop_time) {
            if ($stop_time == 0) {
                ...
                posix_kill($pid, SIGHUP);
                Lib\Task::add(
                    self::KILL_WORKER_TIME_LONG, array('\Man\Core\Master', 'forceKillWorker'), array($pid), false
                );
                break;
            }
        }
    ...
    protected static function updateStatusToShm() {
        ...
        return shm_put_var(
            self::$shmId, self::STATUS_VAR_ID,
            array_merge(self::$serviceStatusInfo, array('pid_map' => self::$workerPidMap))
        );
    }
